...
app.post('/api/sms-receiver', async (req, res) => {
  try {
    // Har xil dasturlardan keladigan xabarlarni tutish (message, body, text)
    ...
    // Secret bo'lsa tekshiradi, bo'lmasa o'tkazib yuboradi (SMS forwarder xato bermasligi uchun)
    if (secret && secret !== SMS_SECRET_KEY) {
      console.warn('⚠️ [SMS] Noto\'g\'ri secret key');
      return res.status(403).json({ success: false, error: 'Invalid secret' });
    }

    // OTP kodi yoki test xabar bo'lsa e'tibor bermaydi
    ...
    // Summani SMS matnidan aniq ajratib olish (Bank SMS lari uchun)
    let amount = 0;
    ...
    updateDb((db) => {
      // Eng oxirgi 'pending' holatdagi va summasi mos keladigan depozitni topadi
